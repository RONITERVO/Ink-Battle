...
import { attackMotion, REST } from "../../src/mr/combat-motion.js";

// Art contact sheets exercise the actual shipped models, at fixed poses and light.
window.drawPencilGallery = (kind = "armies", animated = false) => {
  ...
  let seconds = 0;
  const motion = (period) =>
    animated ? attackMotion(period - (seconds % period), period, true) : REST;
  ...
  const draw = () => {
    ...
    if (kind === "armies") {
      for (const [age, data] of AGES.entries()) {
        ...
        for (let i = 0; i < 3; i++) {
          posed(0.2 + i * 0.43, y, () =>
            unitModel(batch, age, i, {
              x: 0.2 + i * 0.43,
              y,
              scale: 1.24,
              motion: motion(data.units[i].attackSpeed),
            }),
          );
          labels.push({
          ...
          posed(1.64 + i * 0.43, y, () =>
            cannonModel(
              ...
              undefined,
              false,
              motion(data.turrets[i].attackSpeed),
            ),
          );
          labels.push({
  ...
  if (animated) {
    // Manual scrubbing is deterministic; the review page can also play in real
    // time. It uses shipped models and periods, never changes game state.
    window.poseCombatGallery = (time) => {
      seconds = time;
      draw();
    };
    window.playCombatGallery = () => {
    ...
    window.stopCombatGallery = () => renderer.setAnimationLoop(null);
  } else {
    batch.dispose();
    renderer.dispose();
  }
