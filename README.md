...
Do not edit `app/src/main/assets`. Android WebView assets are generated by Gradle from the root web files into `app/build/generated/web-assets` so web and Android stay on the same code path.
...
This repo is ready for GitHub Pages from the repository root. `index.html` is the public entry point, `.nojekyll` disables Jekyll processing, and the launcher redirects to the same playable file used by Android. The PWA shell uses relative paths so project pages such as `https://owner.github.io/repo/` work correctly.
