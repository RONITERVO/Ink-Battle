...
- Gemma turns attach a labeled tactical context image when available; prompt context tells Gemma it controls the blue/right enemy side and the image labels define hit lines, front lines, danger zones, and unit markers. The native bridge configures `visionBackend`, sends the image via `Content.ImageFile`, and retains that file until the native chat resets or closes.
